...
for iternum in $(seq 1 10)
do
	echo "######### running training ##########"
	python $PYTHON_PATH --overwrite_cache --model_type dna --tokenizer_name dna$KMER --model_name_or_path $2 --task_name dnaprom --data_dir $DATA_PATH --output_dir $OUTPUT_PATH --do_train --max_seq_length 101 --per_gpu_eval_batch_size 32 --per_gpu_train_batch_size 32 --learning_rate 2e-4 --num_train_epochs 1 --logging_steps 200 --warmup_percent 0.1 --hidden_dropout_prob 0.1 --overwrite_output_dir --weight_decay 0.01 --n_process 8
